...
module.exports = function (
	...
	return async function (event, message) {
		...
		async function onStart() {
			...
			if (body.trim() === prefix.trim()) {
				const prefixOnlyResponses = [
					...
					"This is my prefix only, try -f a beautiful cat",
			...
			if (getCoolDown == null || isNaN(getCoolDown))
				getCoolDown = 1;
			...
			if (timestamps[senderID]) {
				...
				if (dateNow < expirationTime)
					return await message.reply(
						utils.getText({ lang: langCode, head: "handlerEvents" }, "waitingForCommand", ((expirationTime - dateNow) / 1000).toString().slice(0, 3))
					);
			}
		...
		async function presence() { /* Reserved for presence events */ }

		async function read_receipt() { /* Reserved for read receipt events */ }

		async function typ() { /* Reserved for typing events */ }
