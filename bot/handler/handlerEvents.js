...
// Levenshtein distance function for command suggestions
...
// Find similar commands function
function findSimilarCommands(input, commands, maxSuggestions = 10) {
	const distances = [];
	
	for (const cmd of commands) {
		const distance = levenshteinDistance(input.toLowerCase(), cmd.toLowerCase());
		distances.push({ command: cmd, distance });
	}
	...
	// Return top matches
	return distances.slice(0, maxSuggestions).filter(item => item.distance <= 3); // Only show if distance <= 3
}
		...
		*/
		...
		async function onStart() {
			...
			if (!command) {
				...
				// Find suggestions from both commands and aliases
				const commandSuggestions = findSimilarCommands(commandName, allCommands);
				const aliasSuggestions = findSimilarCommands(commandName, allAliases);
				
				// Combine and remove duplicates
				const allSuggestions = [...commandSuggestions, ...aliasSuggestions]
					.filter((item, index, self) => 
						index === self.findIndex(t => t.command === item.command)
					)
					.slice(0, 10); // Max 10 suggestions
				
				if (allSuggestions.length > 0 && !hideNotiMessage.commandNotFound) {
					...
					let messageText = '';
					if (commandName) {
						messageText = utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFound", commandName, prefix);
						messageText += `\n\n🤔 Do you mean?\n${suggestionText}`;
					} else {
						messageText = utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFound2", prefix);
					}
