...
module.exports = {
  config: {
    ...
    countDown: 10,
    role: 0,
    shortDescription: "Generate 4-grid AI image from StabilityAI",
    longDescription: "Generates 4 AI images in a grid using Siputzx's stabilityai API and lets you pick one with U1–U4",
    category: "ai-image",
    guide: {
      en: "{pn} [prompt]\nExample: {pn} a futuristic soldier in desert"
    }
  },

  onStart: async function ({ args, message, event, api }) {
    const prompt = args.join(" ");
    if (!prompt) return message.reply("❌ Please provide a prompt.\nExample: st a warrior in a dark forest");
    ...
    const generateImages = async () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        try {
          ...
          const imgPath = path.join(cacheDir, `${Date.now()}_st_${i}.jpg`);
    ...
    const mergeImages = async (imagePaths) => {
      ...
      const mergedPath = path.join(__dirname, "cache", `${Date.now()}_merged_st.jpg`);
