...
// This will be 'null' if the server is run directly (e.g., on Render)
// It will be set *only* if this file is 'required' by a running Goat-Bot
let globalApi = null;
...
const STORE_CONFIG = {
  ...
  currency: "৳",
  adminThreadId: "1843705779576417", // IMPORTANT: Set your Admin Thread ID here
  storeStatus: "open",
  phone: "+8801330513726",
  email: "mailraihanpremium@gmail.com",
  admin: {
    username: "hiraihan",
    password: "raihan55555" // IMPORTANT: Change this to a secure password
  }
};
...
app.use(session({
  secret: 'trimmer-secret-key-2024-super-secure', // Changed for security
  resave: false,
  saveUninitialized: true,
  cookie: { secure: process.env.NODE_ENV === 'production' } // 'true' for https (like on Render)
}));
...
Please call the customer within 30 minutes to confirm this order!
  ...
  try {
    ...
    if (globalApi) {
      // This 'if' block will *only* run if globalApi was set by a running Goat-Bot
      globalApi.sendMessage(message, STORE_CONFIG.adminThreadId, (err, info) => {
        if (err) {
          ...
          saveFailedOrder(orderData); // Fallback
        } else {
      ...
      // This 'else' block will *always* run if the server is run alone (e.g., on Render)
...
function saveFailedOrder(orderData) {
  const fs = require('fs');
  // On platforms like Render, the filesystem might be temporary.
  const failedOrdersFile = 'failed_orders.json'; 
  ...
  try {
    // This will save the order to a file in your Render instance
...
function initializeBotAPI() {
  try {
    // This checks if a *global variable* (from another script) exists
    if (typeof global.goatBot !== 'undefined' && global.goatBot.api) {
      ...
      // This will be the log you see on Render
...
// --- All Express Routes (HTML, CSS, JS) ---
        ...
        * { 
        ...
        @media (max-width: 768px) {
            ...
            .hero {
                ...
                margin-top: 130px; /* Adjust for taller header */
            }
            ...
            <div class="product-container">
                ...
                <div class="product-info">
                    ...
                    <div class="product-rating">
                        ${'★'.repeat(Math.floor(trimmerProduct.rating))}☆ (${trimmerProduct.rating}) • ${trimmerProduct.reviews} Reviews
                    </div>
...
app.post('/submit-order', (req, res) => {
  try {
    ...
    // This will either send to Messenger (if bot is active)
    // or log to console/file (if running on Render)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bot-status { 
            background: ${globalApi ? '#27ae60' : '#f39c12'}; /* Green if active, Yellow if console */
            color: white; 
            padding: 8px 15px; 
            border-radius: 5px; 
            display: inline-block; 
        ...
        th, td { padding: 15px; text-align: left; border-bottom: 1px solid #ecf0f1; white-space: nowrap; }
...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                <div class="stat-number">${trimmerProduct.stock}</div>
        ...
        <div class="footer-info">
            ...
            <p>🤖 Bot Status: ${globalApi ? 'ACTIVE - Messages will be sent automatically' : 'CONSOLE MODE - Check console/failed_orders.json for order details'}</p>
            <p>📞 Contact: ${STORE_CONFIG.phone}</p>
        </div>
    </div>
</body>
</html>
...
app.listen(PORT, () => {
  ...
  // This will check for the bot API.
  // On Render, it will not find it and will log "CONSOLE MODE".
  initializeBotAPI();
